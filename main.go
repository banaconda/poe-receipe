...
func (app *HelloTriangleApplication) initWindow() error {
	...
	windowHeight := videoMode.Height / 16 * 9
	monitorX, MonitorY := monitors[0].GetPos()

	window, err := glfw.CreateWindow(windowWidth, windowHeight, "Vulkan Info", nil, nil)

	window.SetAttrib(glfw.Decorated, glfw.False)

	window.SetPos(monitorX+(videoMode.Width-windowWidth)/2, MonitorY+(videoMode.Height-windowHeight)/2)
	...
	app.window = window
	// for event := glfw.GetKeyScancode(); event != nil; event = sdl.PollEvent() {
	// 	switch e := event.(type) {
	// 	case *sdl.QuitEvent:
	// 		break appLoop
	...
	app.window.SetCloseCallback(func(window *glfw.Window) {
		app.window.SetShouldClose(true)
	})

	window.SetFramebufferSizeCallback(func(window *glfw.Window, width int, height int) {
		if width > 0 && height > 0 {
			app.rendering = true
			app.recreateSwapChain()
		} else {
	...
	window.SetIconifyCallback(func(window *glfw.Window, iconified bool) {
		app.rendering = !iconified
	})

	app.loader, err = core.CreateLoaderFromProcAddr(glfw.GetVulkanGetInstanceProcAddress())
	if err != nil {
		return err
	}
...
func (app *HelloTriangleApplication) recreateSwapChain() error {
	...
	// check window is minimized in glfw
	if app.window.GetAttrib(glfw.Iconified) == glfw.True {
		return nil
	}
...
func (app *HelloTriangleApplication) createSurface() error {
	...
	app.surface = *(*khr_surface.Surface)(unsafe.Pointer(&surface))
	return nil
}
...
func (app *HelloTriangleApplication) chooseSwapExtent(capabilities *khr_surface.SurfaceCapabilities) core1_0.Extent2D {
	...
	widthInt, heightInt := app.window.GetSize()
	width := int(widthInt)
